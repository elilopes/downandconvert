...
async function startServer() {
  ...
  async function generateContentWithMultiFallback(
    ai: GoogleGenAI,
    prompt: string,
    ...
    for (const model of geminiModels) {
      try {
        ...
        console.warn(`[AI Fallback] Modelo Gemini ${model} indisponível ou falhou:`, err?.message || err);
      }
    }
  ...
  // =========================================================================
  // CONTROLADOR CENTRAL DE COMUNICAÇÃO COM IA LLM
  ...
  app.post('/api/ai/chat', express.json(), async (req, res) => {
    try {
      ...
      const responseText = await generateContentWithMultiFallback(ai, prompt, systemInstruction);
  ...
  app.post('/api/transcribe/process', async (req, res) => {
    try {
      ...
      const rawResult = await generateContentWithMultiFallback(
        ai,
...
4. Retorne apenas JSON válido sem crases ou markdown adicionais.`;
      ...
      const rawText = await generateContentWithMultiFallback(
        ai,
        prompt,
        'Você é um assistente especialista em comparativos de smartphones. Responda estritamente em JSON válido.',
        'application/json'
      );
